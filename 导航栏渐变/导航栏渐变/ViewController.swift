...
class ViewController: UIViewController , UITableViewDelegate, UITableViewDataSource{
    ...
    override func viewDidLoad() {
        ...
        btn.setImage(UIImage(named: "back_0"), forState: .Normal)
        view.addSubview(btn)
        viewBackBtn = btn
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        if offY >= -64 {
            customBackBtn?.setImage(UIImage(named: "back_1"), forState: .Normal)
            viewBackBtn?.hidden = true
            customTitleLabel?.textColor = UIColor.blackColor()
        } else {
            customBackBtn?.setImage(UIImage(named: "back_0"), forState: .Normal)
